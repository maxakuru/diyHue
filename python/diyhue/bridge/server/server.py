...
_CERT_PATH = '{}/.diyhue/cert.pem'.format(str(Path.home()))
...
class BridgeServer(Thread):
	def __init__(self, emulator, ip, mac):
		...
		self._bridge_config = emulator.bridge_config
		self._ip = ip
		self._mac = mac
	...
	def run(self):
		self.https_server_thread = Thread(target=self.https_server)
		self.https_server_thread.start()
		server_address = ('', 80)
		httpd = ThreadingSimpleServer(server_address, self.BridgeHTTPHandler)
	...
	def https_server(self):
		...
		ctx.load_cert_chain(certfile=_CERT_PATH)
		...
		logger.info('Starting ssl httpd...')
	    ...
	    def do_GET(self):
	        ...
	        # /tradfri setup Tradfri gateway
	            ...
	            if "code" in get_parameters:
	                ...
	                self.emulator.register_tradfri_identity(get_parameters)
	                lights_found = self.emulator.scan_tradfri()
	                self._set_end_headers(
	        ...
	        # /milight setup milight bulb
	            ...
	            if "device_id" in get_parameters:
	                ...
	                milight = self.emulator.register_mi_light(get_params)
	        ...
	        # /hue -> setup Hue bridge
	            ...
	            if "linkbutton" in self.path:
	                ...
	                auth = self.emulator.check_auth(self.headers['Authorization'])
	                if auth:
	                    get_parameters = parse_qs(urlparse(self.path).query)
	                    if "action=Activate" in self.path:
	                        self._set_headers()
	                        self.emulator.activate_link_button()
	                        self._set_end_headers(bytes(
	                        ...
	                        self._set_headers()
	                        tmp_password = str(base64.b64encode(bytes(get_parameters["username"][0] + 
	                                        ":" + get_parameters["password"][0], "utf8"))).split('\'')
	                        success = self.emulator.change_hue_password(tmp_password)
	                        if success:
	                            self._set_end_headers(bytes(
