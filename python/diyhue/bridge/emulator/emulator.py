...
class BridgeEmulator(object):
    def __init__(self, bridge_config):
        ...
        self._ip = self.bridge_config.ip
        self._mac = self.bridge_config.mac
        self._new_lights = {}
        self._controllers = {}
        self._protocols = {
            'native': protocols.native,
            'yeelight': protocols.yeelight,
            'hue': protocols.hue,
            'deconz': protocols.deconz,
            'domoticz': protocols.domoticz,
            'milight': protocols.milight,
            'tradfri': protocols.tradfri,
            'flex': protocols.flex
        }
        ...
        try:
            ...
            if self.bridge_config["deconz"]["enabled"]:
                self._controllers['SSDP'] = SSDPController(emulator=self, 
                                                        ip=self._ip, 
                                                        mac=self._mac)

            self._controllers['scheduler'] = SchedulerController(emulator=self, 
                                                                ip=self._ip,
                                                                mac=self._mac)
    ...
    @new_lights.setter
    def new_lights(self, val):
        self._new_lights = val
    @property
    ...
    def join_all(self):
        """
        Wait for all threads/procs to join
        """
        for key in self._controllers:
    ...
    def send_dgram(self, message, ip, port):
        """
        Send UDP DGRAM to ip:port
        """
        try:
    ...
    def updateGroupStats(self, light): 
        ...
        for group in self.bridge_config["groups"]:
            if ("lights" in self.bridge_config["groups"][group] and 
                light in self.bridge_config["groups"][group]["lights"]):
                    ...
                    for group_light in self.bridge_config["groups"][group]["lights"]:
                        if self.bridge_config["lights"][group_light]["state"]["on"]:
                            any_on = True
                        else:
                            all_on = False
                    self.bridge_config["groups"][group]["state"] = {"any_on": any_on, "all_on": all_on,}
                    self.bridge_config["groups"][group]["action"]["on"] = any_on
